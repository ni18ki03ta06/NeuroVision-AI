...
def generate_pdf_report(result, orig_path=None, gradcam_path=None, patient_name="N/A", ref_id="N/A", physician="N/A", notes="", signature=""):
    ...
    pdf.cell(0, 8, '1. Patient & Scan Metadata', ln=True)
    ...
    pdf.set_font('DejaVu', 'B', 12)
    pdf.cell(0, 8, '2. Classification Results Summary', ln=True)
    for label, val in [
    ...
    pdf.set_font('DejaVu', 'B', 12)
    pdf.cell(0, 8, '3. Neural Network Confidence Breakdown', ln=True)
    ...
    if notes:
        pdf.set_font('DejaVu', 'B', 12)
        pdf.cell(0, 8, '4. Clinical Observations & Notes', ln=True)
        pdf.set_font('DejaVu', '', 9)
    ...
    if signature:
        ...
        pdf.cell(0, 9, f"  ✅ SIGNED & APPROVED BY: {signature} | Date: {datetime.now().strftime('%Y-%m-%d %H:%M')}", 1, 1, 'L', True)
        pdf.ln(4)
    ...
    pdf.cell(0, 10, '🧠 NeuroVision AI — Interpretability & Guidelines', ln=True)
    pdf.ln(2)

    # 6. GradCAM visualization embedded side-by-side
    pdf.set_font('DejaVu', 'B', 12)
    pdf.cell(0, 8, '5. Spatial Interpretability Mapping (GradCAM)', ln=True)
    pdf.ln(2)
    ...
    pdf.set_font('DejaVu', 'B', 12)
    pdf.cell(0, 8, '6. Medical Interpretation', ln=True)
    pdf.set_font('DejaVu', '', 9.5)
    ...
    pdf.set_font('DejaVu', 'B', 12)
    pdf.cell(0, 8, '7. Risk Stratification Explanation', ln=True)
    pdf.set_font('DejaVu', '', 9.5)
    ...
    pdf.set_font('DejaVu', 'B', 12)
    pdf.cell(0, 8, '8. Follow-up Recommendations', ln=True)
    pdf.set_font('DejaVu', '', 9.5)
...
def generate_batch_pdf_report(cases, physician="N/A", notes="", signature=""):
    ...
    for idx, case in enumerate(cases):
        ...
        ref_id = case.get('ref_id', 'N/A')
        orig_path = case.get('orig_path', None)
        gradcam_path = case.get('gradcam_path', None)
        
        # Header
        pdf.set_font('DejaVu', 'B', 16)
        pdf.cell(0, 10, f'🧠 NeuroVision AI — Batch Report | Case {idx + 1}', ln=True)
        pdf.set_font('DejaVu', '', 9)
        pdf.set_text_color(100, 100, 100)
        pdf.cell(0, 6, 'For research use only — not a diagnostic tool', ln=True)
        pdf.set_text_color(0, 0, 0)
        ...
        # 2. Results Summary
        pdf.set_font('DejaVu', 'B', 11)
        pdf.cell(0, 7, '2. Prediction Results Summary', ln=True)
        pdf.set_font('DejaVu', '', 8.5)
        for label, val in [
            ('Primary Tumor Type', result.get('tumor_type', 'N/A')),
            ('Subclass Severity', result.get('severity_class', 'N/A')),
        ...
        pdf.set_font('DejaVu', 'B', 11)
        pdf.cell(0, 7, '3. Clinical Insights & Recommendations', ln=True)
        
        pdf.set_font('DejaVu', 'B', 8.5)
        pdf.cell(0, 5, 'Medical Interpretation:', ln=True)
        pdf.set_font('DejaVu', '', 8.5)
        pdf.multi_cell(0, 4, interpretation)
        pdf.ln(1)
        
        pdf.set_font('DejaVu', 'B', 8.5)
        pdf.cell(0, 5, 'Follow-up Recommendations:', ln=True)
        pdf.set_font('DejaVu', '', 8.5)
        for idx, rec in enumerate(recommendations):
            pdf.cell(5, 4.5, f'{idx+1}.')
    ...
    if signature:
        pdf.ln(2)
        pdf.set_font('DejaVu', 'B', 11)
        pdf.cell(0, 6, 'Batch Approvals & Digital Sign-off', ln=True)
        pdf.set_fill_color(220, 245, 220)
        pdf.cell(0, 9, f"  ✅ BATCH SIGNED & APPROVED BY: {signature} | Date: {datetime.now().strftime('%Y-%m-%d %H:%M')}", 1, 1, 'L', True)
